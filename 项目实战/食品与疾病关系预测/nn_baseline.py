...
# 数据读取
test_food = pd.read_csv('./data/初赛A榜测试集/preliminary_a_food.csv')
test_sub = pd.read_csv('./data/初赛A榜测试集/preliminary_a_submit_sample.csv')
train_food = pd.read_csv('./data/训练集/train_food.csv')  # 食物特征
...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(train, train.related)):
    ...
    for e in range(1, 16):
        for i, (x1, x2, x3, x4, x5, y) in enumerate(train_loader):
            model.train()
            x1, x2, x3, x4, x5, y = x1.cuda().long(), x2.cuda(), x3.cuda(), x4.cuda(), x5.cuda(), y.cuda(0)
        ...
        for (x1, x2, x3, x4, x5, y) in valid_loader:
            valid_acc, valid_loss = evaluate_accuracy(x1.cuda(), x2.cuda(), x3.cuda(), x4.cuda(), x5.cuda(), y.cuda(),
                                                      model, criterion)
            acc_avg += valid_acc / n
        ...
        if best_acc < acc_avg:
            ...
            torch.save(model.state_dict(), f'./save/best_fold{fold_ + 1}.model')
...
for n in range(1, 5 + 1):
    ...
    model.load_state_dict(torch.load(f'./save/best_fold{n}.model'))
    model.eval()
